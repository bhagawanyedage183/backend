...
const ReviewForm = (props) => {
  ...
  React.useEffect(() => {
    ...
    if (fData.error || fData.success) {
      console.log("ReviewForm message:", fData.error || fData.success);
  ...
  const ratingUserList = data.singleProductDetail && data.singleProductDetail.pRatingsReviews
    ? data.singleProductDetail.pRatingsReviews.map(
        (item) => {
  ...
  return (
    <Fragment>
      ...
      {data.isAuthenticated && data.user && ratingUserList.includes(data.user._id) ? (
