  ...
  const fetchData = async () => {
    ...
    setTimeout(() => {
      if (responseData && responseData.Categories) {
        // Sort categories to have parent categories first
                    ...
                    {categories
                      .filter(
                        (subCat) =>
                          ...
                          subCat.parentCategory._id === parentCat._id
                      )
                      .map((subCat) => (
                        <CategoryTable
                          key={subCat._id}
...
const CategoryTable = ({ category, deleteCat, editCat, isSubCategory }) => {
  return (
    <Fragment>
      <tr className={isSubCategory ? "bg-gray-50" : ""}>
        <td className={`p-2 text-left ${isSubCategory ? "pl-8" : ""}`}>
